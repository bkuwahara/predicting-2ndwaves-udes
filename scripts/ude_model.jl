...
train_start = 3
train_split = train_start:train_start + div(train_length, sample_period) - 1
test_split = train_start+div(train_length, sample_period):size(all_data, 2)

train_data = all_data[:, train_split]
test_data = all_data[:, test_split]

all_tsteps = range(train_start, step=sample_period, length=size(all_data,2))
...
network2 = Lux.Chain(
	Lux.Dense(1+num_indicators=>hidden_dims, gelu), Lux.Dense(hidden_dims=>hidden_dims, gelu), Lux.Dense(hidden_dims=>num_indicators))
# network3 = Lux.Chain(
	# Lux.Dense(2=>hidden_dims, tanh), Lux.Dense(hidden_dims=>num_indicators))

# UDE
function ude(du, u, p, t)
	du[1] = -u[1]*u[2]*network1(u[3:end], p.layer1, st1)[1][1]
	du[2] = -du[1] - recovery_rate*u[2]
	du[3] = network2(u[2:3], p.layer2, st2)[1][1] #network2([u[3]], p.layer2, st2)[1][1] - 
	nothing
end
...
function predict(θ, tspan; u0=u0, saveat=sample_period)
	...
	Array(solve(prob, Tsit5(), saveat=saveat))
end
...
function l_layer2(p, Xs)
	...
	for X in Xs
		...
		dM_min = network2([I; mobility_min], p, st2)[1][1]
		l6 += relu(-dM_min)

		# Tending towards M=mobility_baseline when I == ΔI == 0
		dM1_M = network2([0; M], p, st2)[1][1]
		dM2_M = network2([0; M+ϵ], p, st2)[1][1]
		...
		dM1_I = network2([I; M], p, st2)[1][1]
		dM2_I = network2([(I+ϵ); M], p, st2)[1][1]
...
function train_combined(p, tspan; maxiters = maxiters, loss_weights = ones(5), halt_condition=l->false, η=1e-3)
	...
	for iter in 1:maxiters
		...
		layer1_losses, back1 = pullback(θ -> l_layer1(θ, [X[end] for X in Xs]), p.layer1)
...
p1, losses1, loss_weights = train_combined(p_init, (t_train[1], t_train[end]/3); maxiters = 2500, loss_weights = 100*ones(5))
